...
import {
  MLB_INNER_DEVELOPMENT_SECOND_REAL_REGISTRATION_AUTHORIZATION,
  registerAuthorizedMLBInnerDevelopmentSecondRealCandidate,
  type MLBInnerDevelopmentSecondRealCandidateRegistrationInput,
} from '@/prediction/mlb/mlb-inner-development-second-real-candidate-registration';
import {
  computeMLBInnerCandidateRecipeFingerprint,
} from '@/prediction/mlb/mlb-train-only-inner-development-evaluator';

const TEST_REPO_ROOT = '/tmp/mre-second-real-registration-test';

function makeInput(
  overrides: Partial<MLBInnerDevelopmentSecondRealCandidateRegistrationInput> = {},
): MLBInnerDevelopmentSecondRealCandidateRegistrationInput {
  return {
    ...
    authorization: overrides.authorization ?? MLB_INNER_DEVELOPMENT_SECOND_REAL_REGISTRATION_AUTHORIZATION,
  };
}

describe('Phase 8V-D3-C-E4-B4-I3E-B MLB inner development second real candidate registration guard', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it('1. invalid authorization fails closed without calling generic registration', async () => {
    ...
    const result = await registerAuthorizedMLBInnerDevelopmentSecondRealCandidate(input);
  ...
  it('2. authorized call invokes generic registration exactly once with frozen recipe', async () => {
    registrationMock.mockResolvedValue({
      ...
      value: {
        ...
        recipeFingerprint: MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE_FINGERPRINT,
    ...
    const [repositoryRoot, genericInput] = registrationMock.mock.calls[0] as [
      string,
      {
        candidateRecipe: typeof MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE;
    ...
    expect(genericInput.candidateRecipe).toBe(MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE);
  ...
  it('4. public input type does not expose recipe override fields', () => {
    const input = makeInput();
    const keys = Object.keys(input) as readonly (keyof MLBInnerDevelopmentSecondRealCandidateRegistrationInput)[];

    expect(keys).toEqual(['repositoryRoot', 'registrationTimestamp', 'attemptTimestamp', 'authorization']);
  });

  it('5. candidate 002 fingerprint recomputes to expected constant', () => {
    ...
    if (fingerprintResult.ok) {
      expect(fingerprintResult.fingerprint).toBe(MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE_FINGERPRINT);
      expect(fingerprintResult.fingerprint).toBe('c1a0bbbbef214f99d434332a2ab172444468e78f9314b6c133d649b9e3d9c0cc');
    }
  });
  ...
  it('6. caller cannot redirect wrapper to candidate 001', async () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.candidateRecipeId).toBe(MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE_ID);
    }
    ...
    expect(genericInput.candidateRecipe.candidateRecipeId).toBe(MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE_ID);
    expect(genericInput.candidateRecipe.candidateRecipeId).not.toBe(MLB_INNER_DEVELOPMENT_FIRST_REAL_CANDIDATE_RECIPE_ID);
  });

  it('7. authorized wrapper passes candidate 002 as a new distinct recipe identity', async () => {
    registrationMock.mockResolvedValue({
      ok: true,
      value: {
        candidateRecipeId: MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE_ID,
    ...
    expect(genericInput.candidateRecipe.candidateRecipeId).toBe(MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE_ID);
    expect(genericInput.candidateRecipe).toBe(MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE);
    expect(genericInput.candidateRecipe.optimizerConfig.maxIterations).toBe(5000);
  });
  ...
  it('8. duplicate complexityRank 1 is allowed for distinct recipes', () => {
    ...
    expect(MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE.candidateRecipeId).not.toBe(MLB_INNER_DEVELOPMENT_FIRST_REAL_CANDIDATE_RECIPE_ID);
    expect(MLB_INNER_DEVELOPMENT_SECOND_REAL_CANDIDATE_RECIPE_FINGERPRINT).not.toBe(MLB_INNER_DEVELOPMENT_FIRST_REAL_CANDIDATE_RECIPE_FINGERPRINT);
  });
});
