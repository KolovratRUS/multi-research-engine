...
const USAGE = 'npm run prospective:mlb:create-manual-snapshot -- <path-to-json>';

type ArgumentError =
  | 'MANUAL_SCHEDULE_SNAPSHOT_PATH_REQUIRED'
  | 'MANUAL_SCHEDULE_SNAPSHOT_SINGLE_PATH_ONLY';

function parseArgs(argv: string[]): { path: string; error: ArgumentError | null } {
  const args = argv.slice(2);
  if (args.length === 0) {
    return { path: '', error: 'MANUAL_SCHEDULE_SNAPSHOT_PATH_REQUIRED' };
  }
  if (args.length > 1) {
    return { path: '', error: 'MANUAL_SCHEDULE_SNAPSHOT_SINGLE_PATH_ONLY' };
  }
...
try {
  ...
  writeSummary({
    ok: false,
    error: 'MANUAL_SCHEDULE_SNAPSHOT_READ_OR_PARSE_FAILED',
    ...emptyValidationSummary(),
  });
  process.exit(1);
}
...
summary.snapshot = buildScheduleSnapshotFromManualScheduleFile(data as MLBManualScheduleFile);
writeSummary(summary);
