...
The Phase 4P local-only MLB manual week lock CLI golden outputs are documented in `docs/mlb-manual-week-lock-cli-golden-output.md`. They lock only local lock CLI stdout for static fixtures and do not implement live mode, network schedule ingestion, file-output artifacts, generated run artifacts, or historical fixture changes.
The Phase 4Q planning-only MLB manual week lock file-output mode is documented in `docs/mlb-manual-week-lock-file-output-plan.md`. It does not implement live mode, network schedule ingestion, file-output artifacts, generated run artifacts, or historical fixture changes.
The Phase 4R local-only MLB manual week lock file-output mode is implemented and documented in `docs/mlb-manual-week-lock-file-output-plan.md`. It requires both explicit file flags, writes only the exact validated `lockedSnapshot`, preserves the Phase 4P no-flag stdout contract, and does not change historical fixture data.
The Phase 4S local-only file-output golden regression coverage is documented in `docs/mlb-manual-week-lock-file-output-golden-tests.md`. It preserves the Phase 4R implementation and Phase 4P no-flag goldens, commits only static golden fixtures, and leaves generated lock artifacts local and uncommitted.
The Phase 4T planning-only locked-week-to-prospective-construction handoff is documented in `docs/mlb-weekly-prospective-research-construction-plan.md`. It plans how a validated locked artifact will produce deterministic pre-game research skeletons without implementing construction or changing Phase 4R/4S lock behavior or goldens.
Phase 4U implemented that handoff as a local, stdout-only command that consumes the exact `lockedSnapshot` artifact, validates it, and emits one deterministic pre-game `pending-research` `FULL` stub per locked game. Phase 4U itself added no file output, used no live/API/web or network schedule ingestion, and left the Phase 4P and Phase 4S lock goldens unchanged.
Phase 4V added byte-for-byte no-flag construction stdout goldens for the valid package and representative invalid locked artifacts. At that phase, Phase 4U behavior and the Phase 4P/4S lock goldens remained unchanged and construction file output was not yet implemented.
Phase 4W planned construction file output in `docs/mlb-weekly-prospective-research-construction-file-output-plan.md`. Phase 4X implements the double-opt-in mode with `--write-file` plus `--output-dir`. It writes the exact inner construction package, keeps file-mode stdout summary-only, refuses overwrite, and leaves Phase 4U no-flag behavior plus all Phase 4V, Phase 4P, and Phase 4S goldens unchanged. Generated construction artifacts remain local, ignored, and uncommitted.

Phase 4Y adds exact static construction file artifact and file-mode stdout summary goldens in `docs/mlb-weekly-prospective-research-construction-file-output-golden-tests.md`. Phase 4X behavior and the Phase 4V, Phase 4P, and Phase 4S goldens remain unchanged. Generated `tmp` artifacts stay ignored and uncommitted and are cleaned after validation.

Phase 4Z is the planning-only first real MLB research-module handoff in `docs/mlb-first-research-module-handoff-plan.md`. It proposes team recent form research from the exact Phase 4X/4Y construction package artifact. The module enriches pregame research and does not predict. It adds no `modelProbability`, pitcher evidence, live/API/web access, network schedule ingestion, generated run artifact, or historical fixture change. The Phase 4V/4Y construction goldens and Phase 4P/4S lock goldens remain unchanged.
...
Phase 5J implemented safe result-derived aggregate metrics behind an explicit local-only mode in `docs/mlb-team-recent-form-result-aggregate-metrics-implementation.md`. It requires `--fixture-evidence-local --aggregate-summaries-local --result-aggregate-metrics-local`, preserves default Phase 5B/5E/5H goldens, and does not add live/API/web access, network schedule ingestion, or historical fixture changes.
