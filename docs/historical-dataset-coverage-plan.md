...
The recommended next safe phase is Phase 4U: implement stdout-only weekly prospective research construction from a locked manual week. It should remain local-only, validate the locked artifact first, emit a deterministic research skeleton, add no file output, use no live/API/web source or network schedule ingestion, commit no generated run artifacts, and change no historical fixture data.
