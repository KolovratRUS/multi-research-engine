...
The Phase 4F planning-only manually supplied MLB schedule-file dry-run plan is `docs/mlb-manual-schedule-file-dry-run-plan.md`, documenting the future user-supplied static schedule file workflow before any authorized ingestion. It does not implement live mode, schedule ingestion, or fixture data changes.
