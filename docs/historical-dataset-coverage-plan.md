...
Phase 4U implements that handoff as a local, stdout-only command that consumes the exact `lockedSnapshot` artifact, validates it, and emits one deterministic pre-game `pending-research` `FULL` stub per locked game. It adds no file output, uses no live/API/web or network schedule ingestion, and leaves the Phase 4P and Phase 4S lock goldens unchanged.
Phase 4V adds byte-for-byte construction stdout goldens for the valid package and representative invalid locked artifacts. It leaves Phase 4U behavior and the Phase 4P/4S lock goldens unchanged, and adds no construction file output.
Phase 4W is the planning-only future construction file-output design in `docs/mlb-weekly-prospective-research-construction-file-output-plan.md`. It does not implement file output. Phase 4U remains unchanged, the Phase 4V construction stdout goldens remain unchanged, and the Phase 4P no-flag and Phase 4S file-output lock goldens remain unchanged.

The recommended next safe phase is Phase 4X: implement double-opt-in file-output mode for constructed weekly research packages. It should write only the exact validated construction package, preserve all no-flag goldens, refuse overwrite, use no live/API/web or network schedule ingestion, commit no generated run artifacts, and make no historical fixture data changes.
