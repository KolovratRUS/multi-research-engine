...
The recommended next safe phase is Phase 4T: plan weekly prospective research construction from a locked manual week. It is planning-only, defines the future handoff without implementation, uses no live/API/web source or network schedule ingestion, commits no generated run artifacts, and changes no historical fixture data.
