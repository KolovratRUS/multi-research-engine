...
The recommended next safe phase is Phase 4W: plan file-output mode for constructed weekly research packages. It is planning-only, with no implementation, no file output yet, no live/API/web or network schedule ingestion, no generated run artifacts committed, and no historical fixture data changes.
