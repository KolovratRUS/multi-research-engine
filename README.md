...
The Phase 4W planning-only construction file-output contract is documented in `docs/mlb-weekly-prospective-research-construction-file-output-plan.md`. It plans a future double-opt-in file mode without implementing file output or changing Phase 4U, the Phase 4V construction stdout goldens, the Phase 4P no-flag lock goldens, or the Phase 4S file-output lock goldens.
Generated lock artifacts are local files and must not be committed.
Phase 4W leaves the Phase 4U implementation, Phase 4V construction stdout goldens, Phase 4P no-flag lock goldens, and Phase 4S file-output lock goldens unchanged. There is still no construction file output. The recommended next safe phase is Phase 4X: implement double-opt-in file-output mode for constructed weekly research packages, writing only the exact construction package after validation, refusing overwrite, using no live/API/web or network schedule ingestion, committing no generated run artifacts, and changing no historical fixture data.
